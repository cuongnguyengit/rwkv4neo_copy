...
def parse_web(url="https://www.geeksforgeeks.org/"):
    ...
    for link in soup.findAll('a'):
        link = link.get('href')
        if check_url(link) and parse_main_url(url) in link:
            links.append(link)
...
list_links = [
    "https://ykhoa.net/",
    "http://tonghoiyhoc.vn/",
    "https://tapchiyhocvietnam.vn/",
    "https://yhoccongdong.com/",
    "https://www.camnangbenh.com/",
    "https://ivie.vn/",
]
...
txt = open("../data/ykhoa.txt", 'a', encoding='utf-8')
...
while len(list_links) > 0:
    ...
    if url in dict_links:
        ...
        continue
    # if str(url).startswith("https://vnexpress.net/"):
    #     wtxt.write(f"{url}\n")
    ...
    for link in links:
        if link not in list_links and link not in dict_links and check_head(link):
